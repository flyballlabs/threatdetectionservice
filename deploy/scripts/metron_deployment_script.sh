...
cmnd=$(mvn clean package -PHDP-2.5.0.0 -DskipTests)
eval "${cmnd}"
ret_code=$?
if [ $ret_code != 0 ]; then
...
scp -P 2222 -i $SSH_PRIVATE_KEY /threatdetectionservice/deploy/config.sh vagrant@127.0.0.1:/tmp
if [ $? -eq 0 ]; then
...
echo "metron : metron bash cmd for deployment scripts"
