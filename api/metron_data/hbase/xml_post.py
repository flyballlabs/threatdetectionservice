...
if issuccessful(request):
	request = requests.delete(hbaseBaseURL + "/" + hbaseTableName + "/schema", headers={"Accept" : "application/json"})
...
content += '<TableSchema name="' + hbaseTableName + '">'
content += '  <ColumnSchema name="' + hbaseCFName + '" />'
content += '</TableSchema>'

request = requests.post(hbaseBaseURL + "/" + hbaseTableName + "/schema", data=content, headers={"Content-Type" : "text/xml", "Accept" : "text/xml"})
...
for filename in os.listdir(sourceDir):
	...
	request = requests.post(hbaseBaseURL + "/" + hbaseTableName + "/fakerow", data=tostring(cellset), headers={"Content-Type" : "text/xml", "Accept" : "text/xml"})
